...
if CLIENT then
	...
	ENT.Tracks = {
		"vj_nz/music/115.mp3",
		"vj_nz/music/Beauty Of Annihilation.mp3",
		"vj_nz/music/Coming Home.mp3",
		"vj_nz/music/Damned.mp3",
		"vj_nz/music/Lullaby Of A Dead Man.mp3",
		"vj_nz/music/Pareidolia.mp3",
		"vj_nz/music/The One.mp3"
	}
	...
	function ENT:Initialize()
		hook.Add("Think",self,function(self)
			...
			if GetConVar("vj_nz_mutator_sndtrk"):GetInt() == 1 then
...
function ENT:Initialize()
	...
	self.AllowBots = GetConVar("vj_nz_mutator_allowbots"):GetInt() == 1
	...
	for _,v in pairs(plys) do
		NZ_CSound(v,"vj_nz/nz/round/round_start.mp3")
		v:ChatPrint("Wave ".. self:GetWave() .. " has started...")
	end

	if self.AllowBots then
		local maxBots = GetConVar("vj_nz_mutator_maxbots"):GetInt()
		local botCount = maxBots != 0 && maxBots or (game.SinglePlayer() && 15 or math.Clamp((game.MaxPlayers() -#plys) /2,1,24))
		...
		if VJ_HasValue(self.Entities,npc) then
			...
			if self.KillsLeft > 0 then
				for _,v in pairs(player.GetAll()) do
					v:ChatPrint("Undead Remaining: ".. self.KillsLeft)
				end
			end
			if self.KillsLeft <= 0 then
				...
				for _,v in pairs(player.GetAll()) do
					NZ_CSound(v,self:GetSpecialRound() && "vj_nz/nz/round/special_round_end.wav" or "vj_nz/nz/round/round_end.mp3")
				...
				timer.Simple(10,function()
					if IsValid(self) then
						if (self:GetWave() +1) %5 == 0 then
							...
							for _,v in pairs(player.GetAll()) do
								NZ_CSound(v,"vj_nz/nz/round/dog_start.wav")
								NZ_CSound(v,"vj_nz/nz/round/special_round_start.wav")
								v:ChatPrint("Wave ".. self:GetWave() .. " has started...")
							end
						else
...
function ENT:FindSpawnPoint(total,data)
	...
	local minDist = data.MinSpawnDist or 800
	...
	for _,v in pairs(ents.GetAll()) do
		if (v:IsNPC() && (v.VJ_NPC_Class != nil && !VJ_HasValue(v.VJ_NPC_Class,"CLASS_ZOMBIE") or v.VJ_NPC_Class == nil)) or (v:IsPlayer() && !VJ_CVAR_IGNOREPLAYERS && v:Alive()) then
	...
	local function SelectNode()
		...
		for _,v in pairs(enemyEnts) do
			...
			-- local vis = self:GetSpecialRound() && false or v:VisibleVec(node.pos)
			local vis = false
...
function ENT:Think()
	...
	if self.NextSpawnAttemptT < CurTime() then
		...
		for _,s in pairs(spawnPoint) do
			...
			local npc = ents.Create(self:GetSpecialRound() && "npc_vj_nz_hound" or "npc_vj_nz_zombie")
			npc:SetPos(s)
			...
			npc.MeleeAttackDamage = npc.MeleeAttackDamage *(self:GetSpecialRound() && 1 or math_Clamp(wave /3,1,5))
			npc.EntitiesToNoCollide = {"npc_vj_nz_zombie","npc_vj_nz_hound"} -- Optimizations
			npc:Spawn()
			npc:Activate()
			...
			npc.AnimMovementType = wave > 3 && math.random(1,2) or wave > 8 && math.random(1,3) or wave > 12 && 3 or 1
