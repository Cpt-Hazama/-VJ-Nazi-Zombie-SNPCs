...
function ENT:CustomOnThink_AIEnabled()
	if self.UseMovementOverride == true then
		...
		if self:Health() < 40 then
			...
			if GetConVarNumber("vj_nzwavetype") == 2 then // Later Waves
				self.AnimTbl_Walk = {ACT_WALK}
				self.AnimTbl_Run = {ACT_RUN}
			elseif GetConVarNumber("vj_nzwavetype") == 1 then // Early Waves
				self.AnimTbl_Walk = {ACT_WALK}
				self.AnimTbl_Run = {ACT_WALK}
			elseif GetConVarNumber("vj_nzwavetype") == 3 then // Always Sprint
				self.AnimTbl_Walk = {ACT_WALK}
				self.AnimTbl_Run = {ACT_SPRINT}
			end
		end
